...
int main(void)
{
    ...
    void *responsSocket = zmq_socket(context, ZMQ_REP);         //ZMQ_REP - ставит потоки в справедливую очередь которая работатет на полную(п1 - 16, п2 - 2, п3 - 6, п4 - 8. Возьмет п2 и другие потоки до макс 12 пакетов в сек)

    char adress[25];
    printf("Enter bank's adress: ");
    ID bank;
    scanf("%lf", &bank);
    sprintf(adress, "%s%f", "tcp://*:", bank);

    zmq_bind(responsSocket, adress);

    while (1) {
        signal(SIGINT, block_func);             // Ctrl + C
        signal(SIGTSTP, unblock_func);          // Ctrl + Z
        if (!flag) {
