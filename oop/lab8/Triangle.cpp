...
Triangle::Triangle(std::istream &is) 
{
	...
	is >> bi;
	is >> ci;
	while (!(ai + ci > bi && ai + bi > ci && ci + bi > ai && ai > 0 && bi > 0 && ci > 0)){
		std::cout << "Wrong sides! Enter again." << '\n';
		is >> ai;
		is >> bi;
		is >> ci;
	}
...
void Triangle::Print()
{
	std::cout << "Triangle: " << *this << "; S=" << Sq() << std::endl;
	return;
}
...
std::istream& operator>>(std::istream& is, Triangle& obj) 
{
	...
	if (max(ai, max(bi, ci)) >= min(bi, ci)+min(ai, max(bi, ci))){
		...
		std::cout << "Wrong sizes! Triangle not changed!" << std::endl;
	}
	return is;
}
